...
class SparseMemory(nn.Module):
  ...
  def __init__(
    ...
    self.I = (1 - T.eye(self.c).unsqueeze(0))  # (1 * n * n)
    ...
    if hidden is None:
      if self.gpu_id!=-1:   
        hidden = {
            ...
            'memory': T.zeros(b, m, w).fill_(δ).cuda(),
            'visible_memory': T.zeros(b, c, w).fill_(δ).cuda() ,
            'read_weights': T.zeros(b, m).fill_(δ).cuda(),
            'write_weights': T.zeros(b, m).fill_(δ).cuda(),
            'read_vectors': T.zeros(b, r, w).fill_(δ).cuda(),
            'least_used_mem': T.zeros(b, 1).fill_(c + 1).cuda().long(),
            'usage': T.zeros(b, m).cuda(),
            'read_positions': T.arange(0, c).expand(b, c).cuda().long()
        }
      else:
        hidden = {
            # warning can be a huge chunk of contiguous memory
            'memory': T.zeros(b, m, w).fill_(δ),
            'visible_memory': T.zeros(b, c, w).fill_(δ) ,
            'read_weights': T.zeros(b, m).fill_(δ),
            'write_weights': T.zeros(b, m).fill_(δ),
            'read_vectors': T.zeros(b, r, w).fill_(δ),
            'least_used_mem': T.zeros(b, 1).fill_(c + 1).long(),
            'usage': T.zeros(b, m),
            'read_positions': T.arange(0, c).expand(b, c).long()
        }
      ...
      if erase:
        ...
        hidden['read_positions'] = T.arange(0, c).expand(b, c).long()
