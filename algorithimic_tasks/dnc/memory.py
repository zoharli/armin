...
class Memory(nn.Module):
  ...
  def __init__(self, input_size, mem_size=512, cell_size=32, read_heads=4, gpu_id=-1, independent_linears=True):
    ...
    self.memory_bias=nn.Parameter(torch.zeros(1,m,w))
    self.link_bias=nn.Parameter(torch.zeros(1,m,m))
    self.prece_bias=nn.Parameter(torch.zeros(1,1,m))
    self.readw_bias=nn.Parameter(torch.zeros(1,r,m))
    self.writew_bias=nn.Parameter(torch.zeros(1,1,m))
    self.usage_bias=nn.Parameter(torch.zeros(1,m))
  ...
  def reset(self, batch_size=1, hidden=None, erase=False):
    ...
    if hidden is None:
      return {
          'memory': T.zeros(b, m, w),
          'link_matrix': T.zeros(b, 1, m, m),
          'precedence': T.zeros(b, 1, m) ,
          'read_weights': T.zeros(b, r, m) ,
          'write_weights': T.zeros(b, 1, m),
          'usage_vector': T.zeros(b, m)
      }
    else:
      hidden['memory'] = hidden['memory'].detach()
      hidden['link_matrix'] = hidden['link_matrix'].detach()
      hidden['precedence'] = hidden['precedence'].detach()
      hidden['read_weights'] = hidden['read_weights'].detach()
      hidden['write_weights'] = hidden['write_weights'].detach()
      hidden['usage_vector'] = hidden['usage_vector'].detach()
