...
#        self.reset()
    ...
    def reset(self, batch_size=1,cuda=True):
        ...
        for i in range(len(self.heads)):
            prev_weight = torch.zeros([batch_size, self.memory.n]).cuda().double()
            self.prev_head_weights.append(prev_weight)
        self.prev_reads = []
        for i in range(self.num_heads):
            prev_read = torch.zeros([batch_size, self.memory.m]).cuda().double()
    ...
    def forward(self, in_data):
        ...
        #modified from original NTM to aovid nan loss
        controller_c_state=torch.nn.functional.normalize(controller_c_state)
        read_data = []
        head_weights = []
